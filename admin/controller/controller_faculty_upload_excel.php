...
<?php
	...
	*/
	class controller_faculty_upload_excel extends controller
	{
		...
		{
			# code...
			if (isset($_POST["upload-file-excel"])){
			...
			for ($row = 2; $row <= $highestRow; $row++){ 
			    ...
			    //  Insert row data array into your database of choice here
			   	foreach ($rowData as $rowDataChild) {
			   		...
			   		if ($this->model->num_rows("select name from teacher where pk_code_teacher='$code'") == 0){
			   			// chèn giáo viên đó vào database
			   			$pass = md5($code);
			   			$email = $rowDataChild[3];
			   			$this->send_mail($code,$code,$email);
			   			$this->model->execute("insert into teacher(pk_code_teacher,name,password,faculty,mail) VALUES('$rowDataChild[0]','$rowDataChild[1]','$pass','$rowDataChild[2]','$rowDataChild[3]')");
			   		}
			   	}
			}
		}
		...
		public function send_mail($username,$pass,$email)
		{
			...
			$mail->Password   = "21113695"; //Mật khẩu của gmail
			...
			$mail->MsgHTML("Username: " . $username ."<br>"."Password: ". $pass . "<br>". "Bạn hãy truy cập vào link sau đăng nhập và cập nhật tài khoản"); //Nội dung của bức thư.
